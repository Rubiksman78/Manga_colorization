...
gen1 = Generator(8).to(device)
gen2 = Generator(8).to(device)
...
def train(epochs):
    ...
    for epoch in range(epochs):
        ...
        for i,data in progress_bar:
            ...
            dict = {"Discriminator Loss":f"{disc_loss:.4f}","Generator Loss":f"{gen_loss:.4f}","Identity Loss":f"{identity_loss:.4f}","Gan Loss":f"{gan_loss:.4f}","Cycle Loss":f"{cycle_loss:.4f}"}
